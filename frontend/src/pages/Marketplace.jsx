...
import { Store, ShoppingCart, Sprout, Wheat, Package, Search } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#f8fafc] font-sans pb-20">
      ...
      <div className="bg-[#004d00] py-20 px-4 mb-12 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto flex flex-col items-center justify-center relative z-10 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 rounded-full text-green-100 font-bold mb-6 border border-white/20 backdrop-blur-md">
            <Store size={18} /> {t('marketplace.title', "Farmer's Marketplace")}
          </div>
          <h2 className="text-white text-4xl md:text-6xl font-black mb-4 leading-tight">{t('marketplace.title', "Farmer's Marketplace")}</h2>
          <p className="text-green-100 text-lg font-medium max-w-2xl">{t('marketplace.subtitle', 'Buy fresh produce directly from farmers.')}</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 mb-10 relative z-20 -mt-16">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
             ...
             <div className="flex gap-2 w-full md:w-auto bg-gray-100 p-1.5 rounded-2xl overflow-x-auto shrink-0">
               <button onClick={() => setFilterType('all')} className={`px-6 py-2.5 rounded-xl font-bold transition-colors whitespace-nowrap ${filterType === 'all' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:bg-gray-200'}`}>{t('marketplace.allAssets', 'All')}</button>
               <button onClick={() => setFilterType('ready')} className={`px-6 py-2.5 rounded-xl font-bold transition-colors whitespace-nowrap flex items-center gap-2 ${filterType === 'ready' ? 'bg-orange-500 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-200'}`}><Wheat size={16}/> {t('crops.readySection', 'Ready Crops')}</button>
               <button onClick={() => setFilterType('growing')} className={`px-6 py-2.5 rounded-xl font-bold transition-colors whitespace-nowrap flex items-center gap-2 ${filterType === 'growing' ? 'bg-green-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-200'}`}><Sprout size={16}/> {t('crops.growingSection', 'Growing')}</button>
             </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 pt-4 border-t border-gray-100">
             <div className="flex items-center gap-2">
               ...
               <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 text-sm font-bold text-gray-700 outline-none focus:border-green-500">
                 {['All', 'Grains', 'Vegetables', 'Fruits', 'Spices', 'Other'].map(cat => (
                   <option key={cat} value={cat}>{translateText(cat)}</option>
                 ))}
               </select>
             </div>
        ...
        {(filterType === 'all' || filterType === 'ready') && readyCrops.length > 0 && (
          <div className="mb-12">
            <h3 className="text-2xl font-black text-gray-800 mb-6 flex items-center gap-2"><Wheat className="text-orange-600" size={28}/> {t('crops.readySection', 'Ready to Sell')}</h3>
                ...
                <div key={crop._id} className="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl border border-gray-100 transition-all group flex flex-col relative justify-between">
                  <div className="p-6">
                    <div className="flex justify-between items-start mb-2">
                       <h3 className="text-xl font-black text-gray-800 capitalize">{translateText(crop.crop_name)}</h3>
                       <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-orange-100 text-orange-700">{translateText('Ready')}</span>
                    </div>
                    <div className="space-y-3 mb-6 mt-4">
                       <div className="bg-gray-50 p-3 rounded-2xl flex justify-between items-center text-sm">
                         <span className="text-gray-500 font-bold">{t('crops.available', 'Available')}</span>
                       ...
                       <div className="bg-gray-50 p-3 rounded-2xl flex justify-between items-center text-sm">
                         <span className="text-gray-500 font-bold">{t('crops.priceUnit', 'Price')}</span>
                  ...
                  <div className="p-4 bg-gray-50 border-t border-gray-100">
                    <button onClick={() => handleBuyNow(crop)} className="w-full py-3 bg-[#006400] hover:bg-[#004d00] text-white rounded-xl font-black transition-colors flex items-center justify-center gap-2 shadow-md hover:-translate-y-0.5">
                       <ShoppingCart size={18} /> {t('marketplace.buy', 'Buy Now')}
                    </button>
                  </div>
                </div>
              ))}
        ...
        {(filterType === 'all' || filterType === 'growing') && growingCrops.length > 0 && (
          <div className="mb-12">
            <h3 className="text-2xl font-black text-gray-800 mb-6 flex items-center gap-2"><Sprout className="text-green-600" size={28}/> {t('crops.growingSection', 'Growing Crops')}</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {growingCrops.map(crop => (
                <div key={crop._id} className="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl border border-gray-100 transition-all group flex flex-col justify-between relative border-t-4 border-t-green-500">
                  <div className="p-6">
                    <div className="flex justify-between items-start mb-2">
                       <h3 className="text-xl font-black text-gray-800 capitalize">{translateText(crop.crop_name)}</h3>
                       <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-green-100 text-green-700">{translateText('Growing')}</span>
                    </div>
                    <div className="space-y-3 mb-6 mt-4">
                       <div className="bg-gray-50 p-3 rounded-2xl flex justify-between items-center text-sm">
                         <span className="text-gray-500 font-bold">{t('crops.yield', 'Est. Yield')}</span>
                       ...
                       <div className="bg-gray-50 p-3 rounded-2xl flex flex-col gap-1 text-sm">
                         <span className="text-gray-500 font-bold">{t('crops.harvestDate', 'Harvest Date')}</span>
                       ...
                       <div className="bg-gray-50 p-3 rounded-2xl flex flex-col gap-1 text-sm bg-green-50/50">
                         <span className="text-green-600 font-bold text-xs uppercase">{t('crops.priceUnit', 'Price')}</span>
                  ...
                  <div className="p-4 bg-gray-50 border-t border-green-100">
                    {crop.reserved_qty >= crop.expected_yield_qty ? (
                      ...
                      <button onClick={() => { setBookingModal(crop); setBidForm({ offered_price: crop.price, requested_qty: crop.expected_yield_qty }); }} className="w-full py-3 bg-[#FF9800] hover:bg-[#F57C00] text-white rounded-xl font-black transition-colors flex items-center justify-center gap-2 shadow-md hover:-translate-y-0.5 shadow-orange-500/20">
                          <Package size={18} /> {t('marketplace.buy', 'Book Now')}
                      </button>
                    )}
                  </div>
                </div>
      ...
      {bookingModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-[fadeIn_0.2s_ease-out]">
          <div className="bg-white rounded-[2rem] w-full max-w-md shadow-2xl p-8 border border-gray-100 relative">
            ...
            <p className="text-gray-500 text-sm font-medium mb-6">You are placing a pre-harvest bid for <span className="font-bold text-gray-700">{translateText(bookingModal.crop_name)}</span>. The admin desk will negotiate and finalize your contract.</p>
            ...
            <form onSubmit={handlePlaceBooking} className="space-y-4">
              ...
              <div className="flex gap-3 pt-4 mt-2 border-t border-gray-100">
                ...
                <button type="button" onClick={() => setBookingModal(null)} className="px-6 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-xl font-bold text-sm transition-colors">{t('crops.cancel', 'Cancel')}</button>
              </div>
            </form>
          </div>
        </div>
