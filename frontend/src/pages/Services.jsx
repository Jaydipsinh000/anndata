...
import { User, PhoneCall, MapPin, Briefcase, Plus } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#f8fafc] font-sans pb-20">
      ...
      <div className="bg-[#0f172a] py-20 px-4 mb-10 relative overflow-hidden">
        <div className="max-w-7xl mx-auto text-center relative z-10">
          ...
          <h2 className="text-white text-4xl md:text-5xl font-black mb-4">{t('services.title', 'Agricultural Services Hub')}</h2>
          <p className="text-gray-400 text-lg font-medium max-w-2xl mx-auto">{t('services.subtitle', 'Book professional tractor plowing, drone spraying, harvesting, and soil testing services.')}</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center bg-white p-4 rounded-3xl shadow-sm border border-gray-100 mb-8 gap-4">
           ...
           <button onClick={() => {
             ...
             <Plus size={18} /> {t('services.bookNow', 'Book Service Now')}
           </button>
        </div>
        ...
        {loading ? (
           ...
           <div className="bg-white rounded-[2rem] p-16 text-center shadow-sm border border-gray-100">
             ...
             <h4 className="text-xl font-black text-gray-400">{t('services.noServices', 'No services available currently.')}</h4>
           </div>
        ) : (
             ...
             {filteredServices.map(service => (
               <div key={service._id} className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 hover:shadow-xl transition-all group flex flex-col relative overflow-hidden">
                 ...
                 <div className="flex justify-between items-start mb-4">
                    <div>
                      <span className="text-[10px] font-black uppercase tracking-widest text-blue-600 bg-blue-50 px-2 py-1 rounded mb-2 inline-block">{translateText(service.category)}</span>
                      <h3 className="text-xl font-black text-gray-800 leading-tight">{translateText(service.service_title)}</h3>
                    </div>
                 </div>

                 <p className="text-gray-500 text-sm font-medium line-clamp-2 mb-6">{translateText(service.description) || 'No description provided.'}</p>
                 ...
                 <div className="pt-4 border-t border-gray-100 flex items-center justify-between mb-4">
                    <div>
                      <p className="text-xs text-gray-400 font-bold uppercase tracking-wider">{t('services.rate', 'Service Rate')}</p>
                      <p className="text-xl font-black text-blue-700">₹{service.rate} <span className="text-xs text-gray-500 font-medium normal-case">/ {translateText(service.rate_type.replace('per ', ''))}</span></p>
                    </div>
                 </div>
                 ...
                 <a href={`tel:${service.contact_number || service.provider_id?.mobile}`} className="w-full bg-slate-900 text-white font-bold py-3.5 rounded-xl hover:bg-slate-800 transition-colors flex items-center justify-center gap-2 shadow-md">
                   <PhoneCall size={18} /> {t('services.bookNow', 'Contact Provider')}
                 </a>
               </div>
             ))}
           </div>
