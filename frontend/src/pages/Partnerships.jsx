  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] font-sans pb-20">
      ...
      <div className="bg-gradient-to-r from-[#FF9800] to-[#F57C00] py-16 px-4 mb-4 text-center relative overflow-hidden">
        ...
        <h2 className="text-white text-4xl md:text-5xl font-extrabold mb-2 relative z-10 drop-shadow-md">{t('partnerships.title', 'Farm Partnerships & Investments')}</h2>
        <p className="text-orange-100 font-medium text-lg relative z-10">{t('partnerships.subtitle', 'Collaborate with verified agricultural investors, co-farmers, and suppliers.')}</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10 -mt-10 mb-8">
        ...
        {filteredPartnerships.length === 0 ? (
           <div className="bg-white rounded-3xl p-10 text-center shadow-sm border border-gray-100">
             <h3 className="text-2xl font-black text-gray-800 mb-2">{t('partnerships.noItems', 'No partnership requests active right now.')}</h3>
           </div>
        ) : (
                    ...
                    <div className="bg-orange-50/50 p-6 border-b border-orange-100/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                       <div className="flex-1">
                          <div className="flex flex-wrap gap-2 mb-2">
                             <span className={`text-white text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full ${p.status === 'active' ? 'bg-[#FF9800]' : p.status === 'completed' ? 'bg-green-600' : 'bg-gray-500'}`}>{translateText(p.status)}</span>
                          ...
                          <h3 className="text-2xl font-black text-gray-800">Operation: {translateText(p.assigned_crop)}</h3>
                                   ...
                                   <div key={task._id} className={`relative flex items-start gap-4 transition-all duration-300 ${task.is_completed ? 'opacity-60' : 'opacity-100'}`}>
                                      ...
                                      <div className={`flex-1 p-5 rounded-2xl border transition-all ${task.is_completed ? 'bg-gray-50 border-gray-200' : 'bg-white border-orange-200 shadow-sm'}`}>
                                         <div className="flex justify-between items-start gap-2 mb-2">
                                            <h5 className={`font-bold ${task.is_completed ? 'line-through text-gray-500' : 'text-gray-800'}`}>{translateText(task.title)}</h5>
                                         ...
                                         <p className="text-sm text-gray-600 font-medium leading-relaxed mb-4">{translateText(task.description)}</p>
                                ...
                                <ul className="divide-y divide-gray-100">
                                   {expenses.map((exp, i) => (
                                      <li key={i} className="p-4 flex items-center justify-between hover:bg-gray-50 transition-colors">
                                         <div>
                                            <p className="font-bold text-gray-800 text-sm">{translateText(exp.description)}</p>
