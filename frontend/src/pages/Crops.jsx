...
function Crops() {
  ...
  const fetchCrops = async () => {
    try {
      const res = await fetch('/api/crops/mycrops', {
  ...
  const handleSubmit = async (e) => {
    ...
    if (cropType === 'growing' && form.expected_harvest_date) {
      ...
      if (harvestDate < today) {
        setFormError("Harvest date must be in the future (કાપણીની તારીખ આજથી આગળ હોવી જરૂરી છે)");
        return;
      }
    }
  ...
  if (loading) return <div className="min-h-screen bg-[#f8fafc]"><Navbar /><div className="text-center py-20 text-xl font-bold animate-pulse">Loading Your Portfolio...</div></div>;
